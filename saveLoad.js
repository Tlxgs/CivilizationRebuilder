...
function refreshGameStateFromSave(saveData) {
    ...
    // 4. 恢复资源动态数据 - 先重置所有资源的 amount 为 0
    for (let r in GameState.resources) {
        GameState.resources[r].amount = 0;
        GameState.resources[r].visible = false;
    }
    ...
    // 5. 恢复建筑动态数据 - 先重置所有建筑的 count 为 0
    for (let b in GameState.buildings) {
        GameState.buildings[b].count = 0;
        GameState.buildings[b].active = 0;
    }
    ...
    // 6. 恢复科技研究状态 - 先重置所有科技为未研究
    for (let t in GameState.techs) {
        GameState.techs[t].researched = false;
        GameState.techs[t].challengeCompleted = false;
    }
    if (saveData.techs) {
