const POLICIES_CONFIG = {
    ...
    "能源政策": {
        ...
        min: -25, max: 25, step: 1, defaultValue: 0, unit: "%", costPerUnit: 4,
        desc: "调整能源结构：负值偏向传统能源(石油和蒸汽机)，正值偏向清洁能源（太阳能和核能）",
        getEffects: (value) => ({
            prodFactor: { 
                "太阳能板": (Math.pow(Math.abs(25+value),0.5)-5) * 0.2,
                "裂变反应堆": (Math.pow(Math.abs(25+value),0.5)-5) * 0.15 ,
                "聚变反应堆": (Math.pow(Math.abs(25+value),0.5)-5) * 0.15 ,
                "石油发电厂": (Math.pow(Math.abs(25-value),0.5)-5) * 0.2,
                "蒸汽机": (Math.pow(Math.abs(25-value),0.5)-5) * 0.2
            },
            consFactor: {},
            capFactor: {}
        })
